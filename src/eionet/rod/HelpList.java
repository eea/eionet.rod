...
public class HelpList extends ROEditServletAC {
   protected String setXSLT(HttpServletRequest req) {
      return PREFIX + "helplist.xsl";
   }
   ...
   protected DataSourceIF prepareDataSource(Parameters parameters){
      String querySource = PREFIX + "helplist.xrs";
      String par;
      if(parameters.getParameter("mode").equals("RO"))
         par = "LOCATE('HELP_RO_', T_HELP.PK_HELP_ID) > 0 OR LOCATE('HELP_RA_', T_HELP.PK_HELP_ID)";
      else
         par = "LOCATE('HELP_LI_', T_HELP.PK_HELP_ID) > 0";
      String queryPars[][] = {{"ID", par}};
