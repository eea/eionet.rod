    ...
    public Resolution init() throws ServiceException {
        ...
        if (!RODUtil.isNullOrEmpty(idspatial)) {
            ...
            for (CountryDTO spatial : spatials) {
                out.append("<rod:Locality rdf:about=\"http://rod.eionet.europa.eu/spatial/").append(spatial.getCountryId())
                        .append("\">\n");
                out.append("<rod:localityName>").append(RODUtil.replaceTags(spatial.getName(), true, true))
                        .append("</rod:localityName>\n");
                ...
                List<ObligationDTO> obligations = RODServices.getDbService().getSpatialDao()
                        .getCountryObligationsList(spatial.getCountryId().toString());
                for (ObligationDTO obligation : obligations) {
                    out.append("<rod:providerFor rdf:resource=\"http://rod.eionet.europa.eu/obligations/")
