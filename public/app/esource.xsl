	...
	<div class="main">
   		   <table border="0" cellspacing="0" width="100%" cellpadding="2">
               ...
               <tr>
			      ...
			      <tr>
			      	<td colspan="5" align="center" valign="middle" class="save">
		      			<input class="btn" onclick="save(null,false)" type="button" value="Save changes"/>
					&#160;
	   		   		<input class="btn" onclick="history.back()" type="button" value="Exit"/>
				      </td>
			      </tr>
			</table>
		</div>
...
function changeIssuer(oControl) {
	document.forms["f"].elements["]]><xsl:value-of select='ISSUED_BY_URL/@XPath'/><![CDATA["].value=oControl.options[oControl.selectedIndex].url;
	changed();
}
function add() {
	var elem;
	...
	document.forms["f"].elements["dom-update-mode"].value="A";
}


var lnkParent = document.forms["f"].elements["/XmlData/RowSet[@Name='Source']/Row/SubSet[@Name='LnkParent']/Row/T_SOURCE_LNK/FK_SOURCE_PARENT_ID"].options;
var parentLst = document.forms["f"].parent_list.options;

inclSelect(lnkParent, parentLst);
compField("legislation title", document.forms["f"].elements["/XmlData/RowSet[@Name='Source']/Row/T_SOURCE/TITLE"]);
]]>
		</script>
	</xsl:template> 
